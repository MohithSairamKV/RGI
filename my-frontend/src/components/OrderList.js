...
const OrderList = ({ username }) => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const endpoint = username ? `${API_BASE_URL}/user/main/orders/${username}` : `${API_BASE_URL}/generaluser/orders`;
    ...
    const EditQuantity = async (sku, newQuantity) => {
        try {
            const endpoint = username ? `${API_BASE_URL}/api/edit-quantity/${sku}` : `${API_BASE_URL}/generaluser/edit-quantity/${sku}`;
            const response = await fetch(endpoint, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
            ...
            if (!response.ok) throw new Error('Error saving edited quantity');
        } catch (error) {
            console.error('Error:', error);
        }
    };
    ...
    const DeleteProduct = async (sku) => {
        if (window.confirm(`Are you sure you want to delete the product with SKU ${sku}?`)) {
            try {
                const endpoint = username ? `${API_BASE_URL}/api/delete-order-product/${sku}` : `${API_BASE_URL}/generaluser/delete-order-product/${sku}`;
                const response = await fetch(endpoint, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json'
    ...
    const handleSendOrder = async () => {
        try {
            const endpoint = username ? `${API_BASE_URL}/user/main/orders/send/${orders[0].OrderID}` : `${API_BASE_URL}/generaluser/orders/send/${orders[0].OrderID}`;
            const response = await fetch(endpoint, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
